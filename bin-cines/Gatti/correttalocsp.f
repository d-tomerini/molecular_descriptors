      PROGRAM PROAIMV
CG    version parametrized for I5 dumbbell
C     MMO=1000
C     MPTS=50
C     MCENT=110
C     MPRIMS=2000
...
CGATTI
C    Version july 98
C    to calculate both source and electric field contributions
C    Improved october/november 98
C    to avoid call to subroutine sphere
C    radii of capture sphere either read from input
C    or evaluated once for the first integrated atom and then
C    saved on COMMON/DIRK/
CGATTI
C
...
C          Default = 0.
CG  
C   (10) = Whether to calculate the source function and the electric field 
C          contributions at NPOSO points.
C          Default NPOSO= 0.
C          If NPOSO>0 the points (x,y,z) where to calculate the contributions
C          should be specified on the following cards. In addition each point
C          may be classified with two optional labels ANAM,BNAM. The 
C          associated format is (3(1PE16.8),2A8)
CG
C REAL OPTIONS:
...
C          Default = 0.
CG  
C   (10) = Wheter to calculate the source function and the electric field 
C          contributions at NPOSO points.
C          Default NPOSO= 0.
C          If NPOSO>0 the points (x,y,z) where to calculate the contributions
C          should be specified on the following cards. In addition each point
C          may be classified with two optional labels ANAM,BNAM. The 
C          associated format is (3(1PE16.8),2A8)
C   (11) = whether to read the radii of the capture spheres for all
C          the atoms of the molecule.
C          0: they are calculated automatically for each integrated
C             atom (warning very costly for large molecules)
C          1: they are taken from those calculated previously 
C             from the first integrated atom
C             in a sequence of atom's integrations
C          2: they are read from input
CG
C
C REAL OPTIONS:
...
CGI
      CHARACTER*40 INP,INT,WFN,LOC
C     CHARACTER*8 AT,ATNAM,ANUC
      CHARACTER*8 AT,ATNAM,ANUC,ANAM,BNAM
CGF
      CHARACTER*7 OptStr,IStr,StrIn,UpStr
      CHARACTER*6 PAIM,PMEGA,Gstrng,Gupstr
CGI
      CHARACTER*4 FINP, FWFN, FINT,RSTR,FLOC
CGF
      LOGICAL RHF,ROHF,ABNAT,RNAT
CGI  MaxPrp increased to 600 everywhere in the program
      PARAMETER (MCENT=110, MaxCrt=20,MxBcrt=10,MaxPhi=200,MaxTht=200,
     $MaxRad=200,MaxPrp=600,MMO=1000)
      PARAMETER (MAXSO=100)
      COMMON/RHOSO/XSO(3,MAXSO),ANAM(MAXSO),BNAM(MAXSO),NPOSO
      COMMON/UNITS/ ISRF,INPT,IOUT,IWFN,IDBG,ILOC
CGF
...
CGI
C     COMMON /LIMITS/RMAX, NATMX,  NFLIM,  NPROPS
      COMMON /LIMITS/ RMAX, NATMX, NFLIM, NPROPS,NPROSO
CGF
      COMMON /ORBTL/ EORB(MMO),PO(MMO),ROOTPO(MMO),NMO
      DIMENSION CRIT(3,MaxCrt),NSRC(4,Mxbcrt),IOPAIM(20),IOPEGA(20),
     $OPAIM(20),OPEGA(20)
      DATA FINP /'.inp'/, FWFN /'.wfn'/, FINT /'.int'/, FLOC /'.loc'/
...
CG
899   FORMAT(' Maximum number of points allowed for the evaluation',/,
     $' of the source function exceeded - Increase MAXSO')
CG
1999  Format(A6)
...
CGI
      ...
      IF (ILEN .EQ. 0) STOP ' usage: proaimv inpfile wfnfile locfile ' 
      CALL MAKNAME(1,INT,ILEN,FINT)
      IF (ILEN .EQ. 0) STOP ' usage: proaimv inpfile wfnfile locfile ' 
      CALL MAKNAME(2,WFN,ILEN,FWFN)
      IF (ILEN .EQ. 0) STOP ' usage: proaimv inpfile wfnfile locfile ' 
      CALL MAKNAME(3,LOC,ILEN,FLOC)
      IF (ILEN .EQ. 0) STOP ' usage: proaimv inpfile wfnfile locfile ' 
CGF
      OPEN (INPT,FILE=INP,status='unknown')
C     OPEN (IOUT,FILE=INT,status='unknown')
      OPEN (IWFN,FILE=WFN,status='unknown')
CGI
      OPEN (ILOC,FILE=LOC,status='unknown')
CFI
C
      WRITE(IOUT,800)
...
CGI
      NPROST=NPROPS
      DO 6000  IKKK=1,10000
      NPROPS=NPROST
      DO 7000  IKK=1,MAXSO
      ANAM(IKK)='        '
7000  BNAM(IKK)='        '
CGF
      READ(INPT,510,END=6001) AT
...
CGI
      IF(NOP.EQ.10.AND.IOPAIM(10).NE.0)THEN
      IF(IOPAIM(10).GT.MAXSO) then
      write(Iout,899)
      Stop
      endif
      DO 200 ISO=1,IOPAIM(10)
      read(inpt,520)XSO(1,ISO),XSO(2,ISO),XSO(3,ISO),ANAM(ISO),BNAM(ISO)
200   write(Iout,*)XSO(1,Iso),XSO(2,ISO),XSO(3,ISO),ANAM(ISO),BNAM(ISO)
      endif
CGF
125   CONTINUE
...
CG
      IF(NOP.EQ.10.AND.IOPEGA(10).NE.0)THEN
      IF(IOPEGA(10).GT.MAXSO) then
      write(Iout,899)
      Stop
      endif
      DO 300 ISO=1,IOPEGA(10)
300   read(inpt,520)XSO(1,ISO),XSO(2,ISO),XSO(3,ISO),ANAM(ISO),BNAM(ISO)
      endif
      IF(NOP.EQ.11.AND.IOPEGA(11).EQ.2)THEN
      read(inpt,*)(toll2(ia),ia=1,ncent)
      Write(Iout,*) 'radii of capture sphere read from input'
      do 301 ISO=1,NCENT
      Write(Iout,*) 'Atom, radius',ISO,TOLL2(ISO)
301   TOLL2(ISO)=TOLL2(ISO)*TOLL2(ISO)
      endif
CG
135   CONTINUE
...
150   CONTINUE
      ...
      END IF
155   CALL INTEG(BETA,IPHIPL,IT,CRIT,N,NRING,NCAGE,NSRC,IMEGA,IPEG)
      CALL RESULT
6000  CONTINUE
6001  WRITE(IOUT,511)
C
...
C
      LOGICAL RHF,ROHF,ABNAT,RNAT
CG
      CHARACTER*8 ANAM,BNAM
      PARAMETER (MAXSO=100)
CG
      PARAMETER (MCENT=110, MMO=1000, MPTS=50,MaxPrp=600)
      COMMON /VALUES/ THRESH1,THRESH2, GAMMA, TOTE
CGI
C     COMMON /LIMITS/RMAX, NATMX,  NFLIM,  NPROPS
      COMMON /LIMITS/ RMAX, NATMX, NFLIM, NPROPS,NPROSO
...
CG
C     COMMON /DIST/ R2(MPTS,MCENT),DX(MPTS,MCENT),DY(MPTS,MCENT),
C    +              DZ(MPTS,MCENT)
      COMMON /DIST/ R2(MPTS,MCENT),DX(MPTS,MCENT),DY(MPTS,MCENT),
     +              DZ(MPTS,MCENT),RCP(MPTS,MAXSO),XCP(MPTS,MAXSO),
     +              YCP(MPTS,MAXSO),ZCP(MPTS,MAXSO)
      COMMON/RHOSO/XSO(3,MAXSO),ANAM(MAXSO),BNAM(MAXSO),NPOSO
CG
      COMMON /C7/  CT(3,3), X, Y, Z, NCRNT
      COMMON /DIRK/ TOLL,TOLL2(MCENT),STP,SIZE,FSTP,CTF,SSTP,NOSEC,
...
CG
      ...
      DO 126 K=1,NPOSO
      KCN=(K-1)*4+1
      FR(J,NPROSO+KCN)=-4.D0*FR(J,4)/RCP(J,K)
      FR(J,NPROSO+KCN+1)=-FR(J,1)*XCP(J,K)/RCP(J,K)**3
      FR(J,NPROSO+KCN+2)=-FR(J,1)*YCP(J,K)/RCP(J,K)**3
      FR(J,NPROSO+KCN+3)=-FR(J,1)*ZCP(J,K)/RCP(J,K)**3
126   CONTINUE
125   CONTINUE
      endif
CG
...
C
      ...
      PARAMETER (MCENT=110,MMO=1000,MPRIMS=2000, NTYPE=20)
      COMMON /ATOMS/ XC(MCENT),YC(MCENT),ZC(MCENT),CHARG(MCENT),NCENT
      COMMON /ORBTL/ EORB(MMO),PO(MMO),ROOTPO(MMO),NMO
...
CGI
      ...
      PARAMETER (MAXSO=100)
CGF
      PARAMETER (MCENT=110,MMO=1000,MPRIMS=2000, MPTS=50, NTYPE=20)
      COMMON /ATOMS/ XC(MCENT),YC(MCENT),ZC(MCENT),CHARG(MCENT),NCENT
      COMMON /ORBTL/ EORB(MMO),PO(MMO),ROOTPO(MMO),NMO
      COMMON /PRIMSA/ COOAMX(MMO),SUMA(MPRIMS),DIVA(MPRIMS),
     +COOA(MPRIMS,MMO),EXXA(MPRIMS),ICTA(MPRIMS),ITPA(NTYPE),NPRIMSA
...
CGI
      ...
      DO 113 I=1,NPOSO
      DO 114 J=1,PTS
      XCP(J,I)=-XYZ(J,1)+XSO(1,I)
      YCP(J,I)=-XYZ(J,2)+XSO(2,I)
      ZCP(J,I)=-XYZ(J,3)+XSO(3,I)
      RCP(J,I)=XCP(J,I)*XCP(J,I)+YCP(J,I)*YCP(J,I)+ZCP(J,I)*ZCP(J,I)
      RCP(J,I)=DSQRT(RCP(J,I))
 114  CONTINUE
 113  CONTINUE
      endif
CGF
...
C
      DOUBLE PRECISION NINE
      PARAMETER (MCENT=110,MMO=1000, MPRIMS0=2100, MPTS0=200, 
     $MPRIMS=2000,MPTSX=30, NTYPE=20)
      COMMON /ATOMS/ XC(MCENT),YC(MCENT),ZC(MCENT),CHARG(MCENT),NCENT
CGI
      COMMON /UNITS/ ISRF,INPT,IOUT,IWFN,IDBG,ILOC
CGF
...
CGI
      PARAMETER (MBUNC=200)
CGF
      PARAMETER (MCENT=110,MMO=1000,MPTS=50,MaxTht=200,MaxPhi=200,
     $MaxPrp=600,MaxRad=200)
...
CGIN
C    $TH(MaxTht),IA(MaxTht),ICT(MaxTht),FLINTS(MaxPrp),IVD(MPTS),
     $TH(MaxTht),IA(MaxTht),ICT(MaxTht),FLINTS(MaxPrp),IVD(MBUNC),
CGFIN
...
CGIN
      if(idiv.gt.mbunc)then
      write(6,*) 'idiv.gt.mbunc: stop', idiv, mbunc
      stop
      endif
CGFIN
      DO 5 J=1,IDIV
...
C
      ...
      END
CGI
C     SUBROUTINE INTEG(BETA,IPHIPL,IT,CRIT,N,NR,NC,NSRC,IMEGA)
      SUBROUTINE INTEG(BETA,IPHIPL,IT,CRIT,N,NR,NC,NSRC,IMEGA,IPEG)
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
      LOGICAL RHF,ROHF,ABNAT,RNAT
      INTEGER TOTINS
      PARAMETER (MMO=1000,MaxTht=200,MaxPhi=200,MaxCrt=20,Mxbcrt=10,
     $MaxPrp=600,MCENT=110)
...
CGI
C     IF(IMEGA.EQ.1)Call SPhere(Beta)
      IF(IMEGA.EQ.1)then
      IF(IPEG.EQ.0)then
      call SPhere(Beta)
      else
...
C
      ...
      PARAMETER (MCENT=110, MNPT=MaxTht*MaxPhi)
      COMMON /C7/ CT(3,3),X, Y, Z,NCRNT
      COMMON /ATOMS/ XC(MCENT),YC(MCENT),ZC(MCENT),CHARG(MCENT),NCENT
      COMMON /S/ THETA(MaxTht),PHI(MaxPhi),SUR(3,MaxTht,MaxPhi)
      COMMON /DIRK/ TOLL,TOLL2(MCENT),STP,SIZE,FSTP,CTF,SSTP,NOSEC,
...
C
      LOGICAL RHF,ROHF,ABNAT,RNAT
      PARAMETER (MMO=1000,MaxTht=200,MaxPhi=200,MaxPrp=600,
     $MxbCrt=10,Mcent=110,MaxRad=200)
      COMMON /C1/  BASSIN(MaxPrp),TMP(MMO*(MMO+1)/2),NAOL
CGI
C     COMMON /LIMITS/RMAX, NATMX,  NFLIM,  NPROPS
      COMMON /LIMITS/ RMAX, NATMX, NFLIM, NPROPS,NPROSO
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
      PARAMETER (MMO=1000,MaxPhi=200,MaxTht=200,MaxPrp=600)
      COMMON /C5/ WP(MaxPhi),WTT(MaxTht)
CGI
C     COMMON /LIMITS/RMAX, NATMX,  NFLIM,  NPROPS
      COMMON /LIMITS/ RMAX, NATMX, NFLIM, NPROPS,NPROSO
...
C
      CHARACTER*80 WFNTTL,JOBTTL,LINE
      CHARACTER*8 ATNAM
      PARAMETER (MCENT=110,MMO=1000,MPRIMS0=2100, NTYPE=20)
      COMMON /ATOMS/ XC(MCENT),YC(MCENT),ZC(MCENT),CHARG(MCENT),NCENT
      COMMON /ORBTL/ EORB(MMO),PO(MMO),ROOTPO(MMO),NMO
      COMMON /STRING/ WFNTTL,JOBTTL,ATNAM(MCENT),NAT
CGI
...
101   FORMAT (1A80)
CGINS
C102   FORMAT (4X,A4,12X,3(I3,17X))
102   FORMAT (4X,A4,12X,I3,16X,I4,17X,I3,17X)
CGFIN
...
CGI
      CHARACTER*8 ATNAM,ANAM,BNAM
CGF
      LOGICAL RHF,ROHF,ABNAT,RNAT
      PARAMETER (MCENT=110, MMO=1000,MaxPrp=600)
CGI
      PARAMETER(MAXSO=100)
      COMMON/RHOSO/XSO(3,MAXSO),ANAM(MAXSO),BNAM(MAXSO),NPOSO
      DIMENSION TX(MAXSO),TY(MAXSO),TZ(MAXSO)
CGF
CGI
C     COMMON /LIMITS/RMAX, NATMX,  NFLIM,  NPROPS
      COMMON /LIMITS/ RMAX, NATMX, NFLIM, NPROPS,NPROSO
...
CGF
     ...
     $ABNAT,RNAT,XGO,YGO,ZGO
      ...
      Data Zero/0.0d0/,One/1.0d0/,Two/2.0d0/
CG
      Data PIG4/-12.56637061/
C     pig4 is 4 times pi-greek
CG
C
      LMO=NMO
      If(idomag.eq.1)lmo=nmo/7
...
CGI
      ...
      DO 600 K=1,NPOSO
      KCN=(K-1)*4+1
      BAS(NPROSO+KCN)=BAS(NPROSO+KCN)/PIG4
      DAX=(-X+XSO(1,K))
      DAY=(-Y+XSO(2,K))
      DAZ=(-Z+XSO(3,K))
      R232=(DAX*DAX+DAY*DAY+DAZ*DAZ)**1.5D0
      TX(K)=CHG*DAX/R232
      TY(K)=CHG*DAY/R232
      TZ(K)=CHG*DAZ/R232
      BAS(NPROSO+KCN+1)=BAS(NPROSO+KCN+1)+TX(K)              
      BAS(NPROSO+KCN+2)=BAS(NPROSO+KCN+2)+TY(K)               
      BAS(NPROSO+KCN+3)=BAS(NPROSO+KCN+3)+TZ(K)                
 600  CONTINUE
      WRITE(IOUT,1415)
1415  FORMAT(/,'************* S O U R C E   C O N T R I B U T I',
     $' O N S ','*************',/)
      DO 700 K=1,NPOSO
      KCN=(K-1)*4+1
      WRITE(IOUT,1420) ANAM(K),BNAM(K),BAS(NPROSO+KCN)
1420  FORMAT(2A8,1PE22.14)
 700  CONTINUE
      WRITE(IOUT,1421)
1421  FORMAT(/,'************ E L. F I E L D  C O N T R I B U T I',
     $' O N S ','************',/)
      WRITE(IOUT,1422)
1422  FORMAT(5X,'Order : EX EY EZ ; ELEX ELEY ELEZ ;',
     $         ' NUEX NUEY NUEZ',/)
      DO 701 K=1,NPOSO
      KCN=(K-1)*4+1
      WRITE(IOUT,1428) ANAM(K),BNAM(K),BAS(NPROSO+KCN+1)
     $                ,BAS(NPROSO+KCN+2),BAS(NPROSO+KCN+3)
      WRITE(IOUT,1423)(BAS(NPROSO+KCN+1)-TX(K)),
     $                (BAS(NPROSO+KCN+2)-TY(K)),
     $                (BAS(NPROSO+KCN+3)-TZ(K))
      WRITE(IOUT,1423) TX(K),TY(K),TZ(K)
1428  FORMAT(2A8,1PE20.12,1X,1PE20.12,1X,1PE20.12)
1423  FORMAT(16X,1PE20.12,1X,1PE20.12,1X,1PE20.12)
 701  CONTINUE
      endif
CGF
      WRITE(IOUT,*)
...
2959    Continue
      ...
      LTOT=(LMO*(LMO+1))/2
      WRITE(ILOC,10000)NAT,LMO,ATNAM(NAT)
      WRITE(ILOC,1700)(PO(I),I=1,LMO)
      WRITE(ILOC,10001)RHF, ABNAT,RNAT,ROHF,NfBeta,IAlpha1
      WRITE(ILOC,1700)(AOM(KKK),KKK=1,LTOT)
10000 FORMAT(2I4,A8)
10001 FORMAT(4L1,2I4) ! jerry
CGF
      If(ABNat) then
        FOOA = Zero
        FOOB = Zero
...
2960        CONTINUE
        DO 2965 I = 1,IAlpha1-1
          DO 2965 J = IAlpha1,LMO
            K=K+1
            FOOA2 = FOOA2-AOM(K)**2
2965        CONTINUE
        DO 2970 I = IAlpha1,LMO
          DO 2970 J = IAlpha1,I
            K=K+1
            HH = Two
            IF(I.EQ.J)HH = One
...
C
      LOGICAL RHF,ROHF,ABNAT,RNAT
      Parameter(MaxTht=200,MaxPhi=200,Mcent=110,MPhiSm=16,MThtSm=8)
      COMMON /C7/ CT(3,3),X, Y, Z,NCRNT
      COMMON /ATOMS/ XC(MCENT),YC(MCENT),ZC(MCENT),CHARG(MCENT),NCENT
      COMMON /DIRK/ Toll,TOLL2(MCENT),STP,SIZE,FSTP,CTF,SSTP,NOSEC,
     $ISECT,ICP,ISTEP,NABMO,Nacc,IDOAOM,IDOMAG,Imagm,nfbeta,RHF,ROHF,
...
CGFIN
      Parameter (MaxTht=200,MaxPhi=200)
      PARAMETER (MCENT=110,MNPT=MaxTht*MaxPhi,MPTS=50)
...
CGIN
C    $DZ0(MPTS),L(MPTS),MPS(MPTS),ITERMT(MNPT),MX(MPTS),XYZT(3),
     $DZ0(MPTS),L(MPTS),MPS(MBUNC),ITERMT(MNPT),MX(MPTS),XYZT(3),
CGFIN
     $Temp(3)
...
C
      IDIV=NNPTH/MPTS
      IF(IDIV.GT.MBUNC)THEN
      write(6,*)'TRUDGE3: IDIV.GT.MBUNC',IDIV,MBUNC
      stop
      endif
