...
C
      ...
      Parameter(MaxAtm=100,MaxOff=600000,MaxCrt=500)
     ...
     $  XYZ(3),EVSAVE(MaxCrt,3),RHOSAV(MaxCrt)
...
1304  FORMAT(' Newton-Raphson Damping Factor Set to ',1PE16.8,
     $' for Non-Nuclear',/, ' Critical Points and ',1PE16.8,
     $' for Nuclear Critical Points')
1310  FORMAT(I4,1X,3(1PE16.8),2A8,2X,'(',I1,',',I2,')',G12.5)
C
...
C
        ...
        If(Icancl.eq.0)Then
        Call Props(XYZ,IFUNC,ICrit,Iwhole,J1,J2,INP,EVSAVE,Rhosav)
        Endif
        Endif
        Goto 30
C
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
      Parameter (MaxOff=600000,MaxAtm=100)
      COMMON CO(MaxOff),IC(MaxOff),NCENT,NMO,NPRIMS
      COMMON /ANG/ ANGLE(3,MaxAtm,MaxAtm)
      COMMON /OFFSET/ ITYPE,ICENT,IMO,IEORB,IE,ICHARG,IXC,IYC,IZC,
     $  IXX, IYY, IZZ,IRR,IR2,IP,IPSI,IGX,IGY,IGZ,IGXX,IGXY,IGXZ,IGYY,
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
      Parameter(MaxOff=600000,MaxAtm=100,MaxStp=141,MinStp=6)
      COMMON CO(MaxOff),IC(MaxOff),NCENT,NMO,NPRIMS
...
C
      Return
      End 
      Subroutine Props(XYZ,IFUNC,ICrit,Iwhole,J1,J2,INP,Evsave,Rhosav)
C
      Implicit Double Precision (A-H,O-Z)
C
      Character*8 Atnam
     ...
     $  WORK(3,3),HVEE(3,3),WVEE(3),Rhosav(maxCrt)
...
C
      ...
      CHARACTER*8 ATNAM
      CHARACTER*4 MODE
      PARAMETER (NTYPE=20,MaxOff=600000,MaxAtm=100)
      COMMON CO(MaxOff),IC(MaxOff),NCENT,NMO,NPRIMS
      COMMON /OFFSET/ ITYPE,ICENT,IMO,IEORB,IE,ICHARG,IXC,IYC,IZC,
     $  IXX, IYY, IZZ,IRR,IR2,IP,IPSI,IGX,IGY,IGZ,IGXX,IGXY,IGXZ,IGYY,
     $  IGYZ,IGZZ,IGXXX,IGXXY,IGXXZ,IGXYY,IGXZZ,IGXYZ,IGYYY,IGYYZ,
